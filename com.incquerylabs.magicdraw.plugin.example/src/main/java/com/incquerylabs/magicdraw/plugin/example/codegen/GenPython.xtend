...
class GenPython {
	...
	def genBlockCode(Class block) '''
	    «FOR imported: queryEngine.blockMustImport.getAllValuesOfimportedBlock(block)»
	        import «imported.genPackageableElementRef»
	    «ENDFOR»
	...
	def genPackageFilePath(Class block) 
		'''«codeOutletRootPath»/«block.package.genPackageableElementPathSteps.join('/')».__init__.py'''
